...
DWORD signatureScan(DWORD base, string sig) {
    vector<char> search = {};
    ...
    int progress = 0;
    for (int a = 0x0; a < 0x3000000; a += 0x500) {
        char epicScan[0x500];
        ReadProcessMemory(hprocess, (LPVOID)(base + a), &epicScan, sizeof(epicScan), 0);
        for (int b = 0; b < 0x500; b++) {
...
start:
    ...
    while (isRunning(pid)) {
        if (GALoaded == 0) {
            char* GABase = GetModuleBase(L"GameAssembly.dll", pid);
            GameAssemblyNp = (DWORD)GABase;
            if (GameAssemblyNp != 0) {
                cout << "Found address of GameAssembly.dll at 0x";
                cout << hex << GameAssemblyNp << "\n";
                GALoaded = 1;
                byte check = 0;
            ...
            DWORD unlockPetsAddress = signatureScan(GameAssemblyNp, "2C 00 74 04 B0 01 5D") + 0x2;
            ...
            DWORD unlockHatsAddress = signatureScan(GameAssemblyNp, "50 00 74 04 B0 01 5D") + 0x2;
            ...
            DWORD unlockSkinsAddress = signatureScan(GameAssemblyNp, "00 00 00 00 74 04 B0 01 5D") + 0x4;
            ...
            DWORD unlockVisorsAddress = signatureScan(GameAssemblyNp, "40 00 74 04 B0 01 5D C3 56") + 0x2;
            ...
            DWORD unlockNameplatesAddress = signatureScan(GameAssemblyNp, "30 00 74 04 B0 01 5D") + 0x2;
